# original file in mlreflect: mlreflect/curve_fitter/results.py
import datetime
import os.path
...
from mlreflect.curve_fitter.minimizer import mean_squared_error
...
class FitResult:
    ...
    def sld_profile(self):
        ...
        return generator.simulate_sld_profiles(
            self.predicted_parameters, progress_bar=False
        )[0]

    @property
    def interpolated_corrected_reflectivity(self):
        return interp_reflectivity(
            self.q_values_prediction, self.q_values_input, self.corrected_reflectivity
        )

    @property
    def curve_mse(self):
        return mean_squared_error(
            np.log10(self.predicted_reflectivity), np.log10(self.corrected_reflectivity)
        ).round(decimals=6)[0]
...
class FitResultSeries:
    def __init__(self, fit_results_list):
        ...
        self.corrected_reflectivity = np.array(
            [fit_result.corrected_reflectivity for fit_result in fit_results_list]
        )
        self.q_values_input = np.array(
            [fit_result.q_values_input for fit_result in fit_results_list]
        )
        self.predicted_reflectivity = np.array(
            [fit_result.predicted_reflectivity for fit_result in fit_results_list]
        )
        self.q_values_prediction = np.array(
            [fit_result.q_values_prediction for fit_result in fit_results_list]
        )
        self.predicted_parameters = pd.concat(
            [fit_result.predicted_parameters for fit_result in fit_results_list]
        )
        self.best_q_shift = np.array(
            [fit_result.best_q_shift for fit_result in self.fit_results_list]
        )
    ...
    def delta_t(self):
        if None in self.timestamp:
            ...
            datetime_list = [
                datetime.datetime.strptime(t, "%a %b %d %H:%M:%S %Y")
                for t in self.timestamp
            ]
            return np.array(
                [(dt - datetime_list[0]).total_seconds() / 60 for dt in datetime_list]
            )

    @property
    def curve_mse(self):
        return np.array(
            [fit_result.curve_variant_log_mse for fit_result in self.fit_results_list]
        )
    ...
    def save_predicted_reflectivity(self, path: str):
        ...
        for fit_result in self.fit_results_list:
            save_path = os.path.join(
                ...
                f"scan{fit_result.scan_number}_{os.path.basename(path)}",
            )
