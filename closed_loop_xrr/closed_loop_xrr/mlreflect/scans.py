...
from .normalization import normalize_to_first, normalize_to_max
from .interpolation import interp_reflectivity
...
class ReflectivityScan:
    ...
    def q(self):
        return (
            ...
            * np.pi
            / self.wavelength
            * np.sin(self.scattering_angle / 2 * np.pi / 180)
        )

    @property
    def corrected_intensity(self):
        try:
            intensity = self._correct_footprint(
                self.raw_intensity, self.scattering_angle
            )
        except ValueError:
