...
def grow_and_stop(target, priors = None, use_mlreflect: bool = False):
    ...
    def do_scans():
        ...
        #return autof_eh1.a2scan(chi,0.05,1.5,gam,0.1,3,191,.5)  
         ...
         ct()
         umv(watt0,6)
         s2=a2scan(chi,0.05,0.1,gam,0.1,0.2,2,1)
         umv(watt0,3)
         s1=a2scan(chi,0.5,1.1,gam,1.,2.2,60,1)
         umv(watt0,1)
         s####=a2scan(chi,1.11,1.5,gam,2.22,3.,38,1)       
         return [s1,s2]
         ct()
         
ct();umv(watt0,6);a2scan(chi,0.05,0.1,gam,0.1,0.2,2,1);umv(watt0,3);a2scan(chi,0.5,1.1,gam,1.,2.2,60,1);umv(watt0,1);a2scan(chi,1.11,1.5,gam,2.22,3.,38,1);ct()     
ct(0.1);umv(watt0,3);f2scan(gam,1.,0.02,chi,0.5,0.01,60,.5);umv(watt0,1);f2scan(gam,2.22,0.02,chi,1.11,0.01,38,.5);ct(0.1)      
         fil
ct();umv(watt0,6);s2=a2scan(chi,0.05,0.1,gam,0.1,0.2,2,1);umv(watt0,3);s1=a2scan(chi,0.5,1.1,gam,1.,2.2,60,1);umv(watt0,1);s1=a2scan(chi,1.11,1.5,gam,2.22,3.,38,1);ct()     
    

         umv(watt0,6)
         s2=a2scan(chi,0.05,0.1,gam,0.1,0.2,2,1)
         umv(watt0,3)
         s1=a2scan(chi,0.5,0.9,gam,1.,1.8,40,1)
         umv(watt0,1)
         s1=a2scan(chi,0.91,1.5,gam,1.82,3.,58,1)       
         return [s1,s2]
         ct()
         ct()
